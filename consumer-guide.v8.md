...
| `text_diff`           | A committed change (**has** `to_version`) or a proposed AI edit (**no** `to_version`) | `{ "file_id": "…", "from_version": <int>, "to_version"?: <int>, "origin": "agent"\|"user", "diff": "<git diff>" }` |
...
**The server owns the document; the client owns the merge.** The server holds the authoritative copy of every open document and assigns it a monotonically increasing integer `version`. Committed changes — the user's manual edits and the AI edits they accept — are relayed to the server as a **git unified diff** (`document_edit`, below) and echoed back as a **committed** `text_diff` carrying `from_version`/`to_version`. You keep an in-memory copy in lock-step: apply each committed diff when its `from_version` equals your current version, then adopt `to_version`. AI *proposals* are a different thing (a `text_diff` with no `to_version`) and never mutate the server copy — see next.

**Edit (propose → accept/reject).** The `DOCUMENT_EDIT` tool does **not** change the document — it **proposes** a change for the user to accept or reject. It emits a `text_diff` with a `diff` and `from_version` but **no `to_version`** — that missing field is how you tell a proposal from a commit. Render the hunks in a review UI (accept/reject, per hunk if you like); nothing is committed on the server. The `tool_result` reports `status: "awaiting_user_decision"` and `proposed: <n>`:

````
client → { "text": "Tighten the intro and drop the disclaimer paragraph" }

server → { "type": "tool_call", "content": { "name": "DOCUMENT_EDIT", "args": { … } } }
server → { "type": "text_diff", "content": {          // ← no to_version: this is a PROPOSAL
             "file_id": "b2c4…",
             "from_version": 1,
             "origin": "agent",
...
server → { "type": "text_diff", "content": {          // ← has to_version: this is a COMMIT
             "file_id": "b2c4…", "from_version": 2, "to_version": 3, "origin": "user",
             "diff": "…", "old_value": "…", "new_value": "…" } }
````

The server applies your diff to its current authoritative copy, bumps the version, audits it, and **echoes a committed `text_diff`** (with `to_version`) carrying the resulting version. Adopt that version. The model is told about the change on its next turn, so it edits against your latest text. For history, `GET /threads/{session_id}` returns each committed change as a `role: "edit"` row (proposals are not recorded — only what the user committed).
...
**Versioning contract.** `version` is owned by the server and only ever increases; clients never send a version. A **committed** `text_diff` (one that carries a `to_version`) applies when its `from_version` matches your current version — adopt `to_version`; if `from_version` is ahead of you, you missed a frame — request a resync. A **proposal** `text_diff` (no `to_version`) carries no version to adopt — it is a pending change you render for accept/reject and only commit by relaying a `document_edit`. When you relay a `document_edit`, just send the diff: the server applies it to its current version and tells you the resulting `to_version` in the echoed `text_diff`. `version` is a per-session working counter, not a stored document revision.
...
text_diff           { file_id, from_version, origin: "agent"|"user", diff, [to_version] }
                    #   to_version present = committed (apply); absent = proposal (accept/reject)
