...
| `text_diff`           | Agent edited a text/markdown document (line-ops)         | `{ "file_id": "…", "base_version": <str\|int>, "ops": [ … ] }`                                     |
...
**Which attachment is being edited.** The `text_diff` event's **`content.file_id` is the file_id of the document being edited** — the same `file_id` you received when the document was delivered (as a `kind: "editable"` attachment). Track delivered `editable` attachments by `file_id`; when a `text_diff` arrives, look that file_id up and apply the ops to that document's in-memory copy. The diff is **not** an attachment — it is delivered only as the `text_diff` event (and recorded as a `tool_result` row for history replay via `/threads`), so `final.attachments` is empty on an edit turn.

**Edit (line-ops diff).** On an edit turn the agent does **not** re-send the whole document. It returns a structured line-ops diff via a dedicated `text_diff` event, which you apply to your in-memory preview (Cursor-style):
...
server → { "type": "text_diff", "content": {
             ...
             "base_version": 1,
             "ops": [
               { "op": "replace", "start_line": 3, "end_line": 5, "text": "A tighter intro paragraph." },
               { "op": "delete",  "start_line": 12, "end_line": 14 },
               { "op": "insert",  "start_line": 20, "text": "## Appendix" }
             ]
           } }
server → { "type": "tool_result", "content": { "name": "KNOWLEDGE_MARKDOWN_EDIT",
             "result": { "applied_ops": 3, "text_diff": { "file_id": "b2c4…", "base_version": 1, "ops": [ … ] } } } }
...
Apply edits from the `text_diff` **event** and from the `tool_result` (which carries the same `text_diff` payload). Diffs are not stored as files — `final.attachments` is empty on an edit turn. For history, `GET /threads/{session_id}` returns each edit as a `tool_result` row containing the `text_diff` payload.

Op semantics (1-based line numbers, applied against `base_version`):

| `op`      | Fields                          | Effect                                                       |
| --------- | ------------------------------- | ------------------------------------------------------------ |
| `replace` | `start_line`, `end_line`, `text`| Replace lines `start_line..end_line` (inclusive) with `text` |
| `insert`  | `start_line`, `text`            | Insert `text` **before** `start_line`                        |
| `delete`  | `start_line`, `end_line`        | Delete lines `start_line..end_line` (inclusive)              |

`base_version` is the document version the diff was computed against. **You own the in-memory document**, including any manual user edits, so reconcile before applying: if your current version no longer matches `base_version`, apply with care (or re-fetch the document attachment and let the user re-issue the edit). The server is stateless about document content — the `text_diff` event and the persisted diff attachment are the whole contract. Apply the ops in the order given.
...
| `editable` | any `text/*` (markdown, code, config, …) | Fetch `url` for the body, render in a preview/editor. This is an **editable plain-text** body — remember its `file_id`; later `text_diff` events target it |
...
text_diff           { file_id, base_version, ops: [{op, start_line, end_line?, text?}] }
