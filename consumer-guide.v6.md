...
This is the contract behind Knowledge Composer **preview** and **edit** (and any future text-document surface — the protocol is text-generic, not markdown-specific).
...
Apply the diff from the `text_diff` **event** (the `tool_result` carries the same payload for history). Diffs are not stored as files — `final.attachments` is empty on an edit turn. For history, `GET /threads/{session_id}` returns each edit as a `tool_result` row containing the `text_diff` payload. `diff` is a standard `git diff` string: feed it to any unified-diff/patch applier.
...
server → { "type": "text_diff", "content": { "file_id": "b2c4…", "from_version": 2, "to_version": 3, "origin": "user", "diff": "…" } }
````
