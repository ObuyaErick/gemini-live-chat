...
| `text_diff`           | A document changed (agent or user); git unified diff     | `{ "file_id": "…", "from_version": <int>, "to_version": <int>, "origin": "agent"\|"user", "diff": "<git diff>" }` |
...
| `{ "type": "document_edit", "content": { "file_id": "…", "diff": "…" } }`      | Relay a manual user edit as a git diff; server owns the version |
...
The `charts` array in `tool_result` is **informational only** — it tells you how many charts were produced and their filenames. Do **not** use those records for rendering; they carry no `url`. The `final.attachments` array is the authoritative source and carries freshly resolved URLs. See [Section 8](#8-attachments-and-chart-rendering) for the rendering walkthrough.
...
**Which document is being edited.** The `text_diff` event's **`content.file_id` is the file_id of the document being edited** — the same `file_id` you received when the document was delivered (as a `kind: "editable"` attachment). Track delivered `editable` attachments by `file_id`; when a `text_diff` arrives, look that file_id up and apply the diff to that document's in-memory copy. The diff is **not** an attachment — it is delivered only as the `text_diff` event (and recorded as a `tool_result` row for history replay via `/threads`), so `final.attachments` is empty on an edit turn.
...
**The server owns the document.** Unlike before, the server holds the authoritative copy of every open document and assigns it a monotonically increasing integer `version`. Every edit — whether the agent's or the user's — is a **standard git unified diff** carried in a `text_diff` event with `from_version`/`to_version`. You keep an in-memory copy in lock-step: apply each diff when its `from_version` equals your current version, then adopt `to_version`.

**Edit (git unified diff).** On an edit turn the agent does **not** re-send the whole document. It returns a git unified diff via a dedicated `text_diff` event, which you apply to your in-memory preview (Cursor-style):

````
client → { "text": "Tighten the intro and drop the disclaimer paragraph" }

server → { "type": "tool_call", "content": { "name": "KNOWLEDGE_MARKDOWN_EDIT", "args": { … } } }
server → { "type": "text_diff", "content": {
             "file_id": "b2c4…",
             "from_version": 1,
             "to_version": 2,
             "origin": "agent",
             "diff": "diff --git a/quarterly-review.md b/quarterly-review.md\nindex 9db7f57..d3612ef 100644\n--- a/quarterly-review.md\n+++ b/quarterly-review.md\n@@ -3,3 +3,1 @@\n-Old intro line one.\n-Old intro line two.\n-Old intro line three.\n+A tighter intro paragraph.\n",
             "old_value": "<full document body before the edit>",
             "new_value": "<full document body after the edit>"
           } }
server → { "type": "tool_result", "content": { "name": "KNOWLEDGE_MARKDOWN_EDIT",
             "result": { "applied": 2, "text_diff": { "file_id": "b2c4…", "from_version": 1, "to_version": 2, "origin": "agent", "diff": "…" } } } }
server → { "type": "delta",  "content": "Done — tightened the intro and removed the disclaimer." }
server → { "type": "final",  "content": "Done — tightened the intro and removed the disclaimer.", "attachments": [] }
````

Apply the diff from the `text_diff` **event** (the `tool_result` carries the same payload for history). Each `text_diff` also carries `old_value`/`new_value` — the full document body before and after the change — so you can hard-set your editor buffer instead of applying the patch if you prefer. Diffs are not stored as files — `final.attachments` is empty on an edit turn. For history, `GET /threads/{session_id}` returns each edit as a `tool_result` row containing the `text_diff` payload. `diff` is a standard `git diff` string: feed it to any unified-diff/patch applier.

**Relaying manual user edits (`document_edit`).** When the user edits the document in your preview/editor surface, send the change back to the server as a git unified diff so its authoritative copy and the model's view stay in sync. **You do not send a version** — the server owns it:
...
server → { "type": "text_diff", "content": { "file_id": "b2c4…", "from_version": 2, "to_version": 3, "origin": "user", "diff": "…", "old_value": "…", "new_value": "…" } }
````

The server applies your diff to its current authoritative copy, bumps the version, and **echoes a confirming `text_diff` with `origin: "user"`** carrying the resulting `to_version`. Adopt that version. (The model is told about the change on its next turn, so it edits against your latest text.)
...
**Versioning contract.** `version` is owned by the server and only ever increases; clients never send a version. On an outbound `text_diff`, apply it when its `from_version` matches your current version and adopt `to_version`; if `from_version` is ahead of you, you missed a frame — request a resync. When relaying a manual `document_edit`, just send the diff: the server applies it to its current version and tells you the resulting `to_version` in the echoed `text_diff`. `version` is a per-session working counter, not a stored document revision.
...
Slots appear in the same order as the `charts` array in the `GENERATE_CHARTS` `tool_result` but the slot content holds an id corresponding the the attachements array. If the model produced no charts (or chart generation failed), no slot markers appear and `final.attachments` is empty.
...
text_diff           { file_id, from_version, to_version, origin: "agent"|"user", diff: "<git diff>" }
...
{ type: "document_edit", content: { file_id, diff } }               # relay a manual user edit (git diff); server owns the version
