...
**Versioning contract.** `version` is owned by the server and only ever increases; clients never send a version. On an outbound `text_diff`, apply it when its `from_version` matches your current version and adopt `to_version`; if `from_version` is ahead of you, you missed a frame — request a resync. When relaying a manual `document_edit`, just send the diff: the server applies it to its current version and tells you the resulting `to_version` in the echoed `text_diff`. `version` is a per-session working counter, not a stored document revision. Saving (`KNOWLEDGE_MARKDOWN_SAVE`) persists the document to Knowledge Composer, which is the source of truth across sessions; on reconnect re-fetch the `editable` attachment to load the saved body rather than relying on the previous session's `version`.
