...
A document can enter preview two ways, both delivering an identical `editable` attachment on `final.attachments`: the agent **retrieves** an existing document, or the agent **creates** a new one from scratch (the model authors the body — no prior document needed). Once open, the document is revised via **edit** — the agent *proposes* a change the user accepts or rejects — and persisted via **save**. Saving is an ordinary tool call — you see a `tool_call`/`tool_result` pair and no document body crosses the wire (the server persists the copy it already holds). Persistence comes in two forms the agent chooses between: **creating a new** knowledge-base node (for a document authored from scratch) and **updating an existing** node (for a retrieved document that was edited — this changes only the body and leaves the node's other fields intact). Some persistence tools target an API, some BigQuery; from your side they all behave identically. Nothing new to handle for create, update, or save beyond the `editable` attachment and tool frames you already render.
...
**On reconnect**, rebuild the document from the `history` timeline: take the `editable` attachment body as version 0 and replay the `role: "edit"` entries for that `file_id` in order (see [§6.2](#62-history-replay-on-reconnect)). The last `to_version` is your current version, and the server has hydrated the matching copy, so your next edit lines up. Saving (create a new node) or updating (an existing one) separately persists the document to the knowledge base, the cross-session source of truth.
...
- Text mode only. In live (voice) mode `ASK_USER` is not offered.
...
GET    /threads                                  (x-winp-token)   # all of the user's sessions (shared, not agent-scoped)
