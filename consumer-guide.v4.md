...
**Relaying manual user edits (`document_edit`).** When the user edits the document in your preview/editor surface, send the change back to the server as a git unified diff so its authoritative copy and the model's view stay in sync:

````
client → { "type": "document_edit", "content": {
             "file_id": "b2c4…",
             "from_version": 2,            // the version you edited on top of
...
The server applies your diff, bumps the version, and **echoes a confirming `text_diff` with `origin: "user"`** carrying the authoritative `to_version`. Adopt that version. (The model is told about the change on its next turn, so it edits against your latest text.)
...
**Versioning contract.** `version` is owned by the server and only ever increases. Apply a `text_diff` only when its `from_version` matches your current version; otherwise wait for the in-flight echo or request a resync. After a save (`KNOWLEDGE_MARKDOWN_SAVE`) the saved content becomes the base version a resumed session opens on, so on reconnect re-fetch the `editable` attachment to get the current body and version rather than replaying old diffs.
...
{ type: "document_edit", content: { file_id, from_version, diff } } # relay a manual user edit (git diff)
